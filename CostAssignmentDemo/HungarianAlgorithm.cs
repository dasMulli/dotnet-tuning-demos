...
public static unsafe class HungarianAlgorithm
{
    ...
    public static int[] FindAssignments(Matrix<float> assignmentCosts)
    {
        ...
        for (var row = 0; row < rows; row++)
        {
            ...
            {
                var cost = (float)assignmentCosts[row, column];
    ...
    private static bool TryFindZero(Storage<float> costs, bool[] rowsCovered, bool[] colsCovered, out Location zeroLocation)
    {
        if (rowsCovered == null)
        {
